...
export default function DevicesPage({ params }: { params: Promise<{ org: string }> }) {
  ...
  return (
    <div className="p-6">
      ...
      <h1 className="text-2xl font-bold mb-2">{decodedOrg}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {devices.map((device) => (
          <Link
            key={device}
            href={`/${encodeURIComponent(decodedOrg)}/${encodeURIComponent(device)}`}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-medium text-zinc-200">{device}</div>
